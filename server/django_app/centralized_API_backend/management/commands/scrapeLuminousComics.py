...
log_directory = "../out/LuminousComics"
log_base_filename = "scrapeLuminousComics"
...
logger = logging.getLogger("LuminousComicsScraper")

class LuminousComicsScraper:
    def scrape_luminous_comics(self):
        # Define URLs for scraping
        url = 'https://luminouscomics.org/series/list-mode/'
        ...
        if books == 'Details not available':
            ...
            # LuminousComics has a blocker, so you can't scrape too quickly
    ...
    def scrape_book_details(self, book_url):
        ...
        try:
            ...
            # Scrape chapters from the correct section
            chapter_section = soup.find('div', class_='bixbox bxcl epcheck')
            if chapter_section:
                chapter_elements = chapter_section.find_all('li', {'data-num': True})
                chapters = {}
            ...
            # logger.info(f'Printing details for Book: {details["title"]}')
            # logger.info(details)
            return details
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'LuminousComics'])
    ...
    # TODO: Fix my horrible implementaitno
...
class Command(BaseCommand):
    help = 'Scrapes books from LuminousComics and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from LuminousComics.

        Executes the scraping process, calculates the duration of the operation, and logs the result.
        """
        logger.info("Starting to scrape LuminousComics")
        ...
        start_time = datetime.datetime.now()
        scraper = LuminousComicsScraper()
        try:
            scraper.scrape_luminous_comics()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeLuminousComics in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeLuminousComics'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for LuminousComics books.")
        except Exception as e:
