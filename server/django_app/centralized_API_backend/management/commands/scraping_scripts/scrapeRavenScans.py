...
log_directory = "../out/RavenScans"
log_base_filename = "scrapeRavenScans"
...
logger = logging.getLogger("RavenScansScraper")

class RavenScansScraper:
    def scrape_raven_scans(self):
        # Define URLs for scraping
        url = 'https://ravenscans.com/manga/list-mode/'
        ...
        try:
            ...
            for element in imptdt_elements:
                ...
                if 'Status' in text:
                    ...
                    # Calling all novels "Light Novel" for consistency. This website calls them "Manhwa Novel"
                    details['novel_type'] = text.replace('Type', '').replace('Manhwa', 'Light').strip()
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'Raven Scans'])
...
class Command(BaseCommand):
    help = 'Scrapes books from RavenScans and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from RavenScans.

        Executes the scraping process, calculates the duration of the operation, and logs the result.
        """
        # logger.info("Starting to scrape RavenScans")
        ...
        try:
            scraper.scrape_raven_scans()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeRavenScans in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeRavenScans'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for RavenScans books.")
        except Exception as e:
