# Since anigliscans and agscomics are both under Animated Glitched Scans
# There are two scripts named as such.
import datetime
import json
import time
import os
...
log_directory = "../out/AnimatedGlitchedScans2"
log_base_filename = "scrapeAnimatedGlitchedScans2"
...
logger = logging.getLogger("AnimatedGlitchedScans2Scraper")

class AnimatedGlitchedScans2Scraper:
    def scrape_animated_glitched_scans(self):
        # Define URLs for scraping
        url = 'https://agscomics.com/series/list-mode/'
    ...
    def scrape_book_details(self, book_url):
        ...
        try:
            ...
            details = {
                ...
                'novel_type': 'Manhwa',
                'novel_source': 'Animated Glitched Scans',
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'Animated Glitched Scans'])
...
class Command(BaseCommand):
    help = 'Scrapes books from Animated Glitched Scans and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from Animated Glitched Scans.
        ...
        start_time = datetime.datetime.now()
        scraper = AnimatedGlitchedScans2Scraper()
        try:
            scraper.scrape_animated_glitched_scans()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeAnimatedGlitchedScans2 in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeAnimatedGlitchedScans2'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for AnimatedGlitchedScans2 books.")
        except Exception as e:
