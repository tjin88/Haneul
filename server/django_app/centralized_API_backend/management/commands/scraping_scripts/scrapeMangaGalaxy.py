...
log_directory = "../out/MangaGalaxy"
log_base_filename = "scrapeMangaGalaxy"
...
logger = logging.getLogger("MangaGalaxyScraper")

class MangaGalaxyScraper:
    def scrape_manga_galaxy(self):
        # Define URLs for scraping
        url = 'https://mangagalaxy.me/series/list-mode/'
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'Manga Galaxy'])
...
class Command(BaseCommand):
    help = 'Scrapes books from MangaGalaxy and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from MangaGalaxy.

        Executes the scraping process, calculates the duration of the operation, and logs the result.
        """
        logger.info("Starting to scrape MangaGalaxy")
        ...
        start_time = datetime.datetime.now()
        scraper = MangaGalaxyScraper()
        try:
            scraper.scrape_manga_galaxy()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeMangaGalaxy in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeMangaGalaxy'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for MangaGalaxy books.")
        except Exception as e:
