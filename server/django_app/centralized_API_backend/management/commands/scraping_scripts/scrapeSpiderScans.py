...
log_directory = "../out/SpiderScans"
log_base_filename = "scrapeSpiderScans"
...
logger = logging.getLogger("SpiderScansScraper")

class SpiderScansScraper:
    def scrape_spider_scans(self):
        # Define URLs for scraping
        url = 'https://spiderscans.xyz/manga/list-mode/'
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'Spider Scans'])
...
class Command(BaseCommand):
    help = 'Scrapes books from Spider Scans and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from Spider Scans.

        Executes the scraping process, calculates the duration of the operation, and logs the result.
        """
        logger.info("Starting to scrape Spider Scans")
        ...
        start_time = datetime.datetime.now()
        scraper = SpiderScansScraper()
        try:
            scraper.scrape_spider_scans()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeSpiderScans in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeSpiderScans'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for SpiderScans books.")
        except Exception as e:
