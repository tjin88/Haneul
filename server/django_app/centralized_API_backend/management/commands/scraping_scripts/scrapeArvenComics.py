...
log_directory = "../out/ArvenComics"
log_base_filename = "scrapeArvenComics"
...
logger = logging.getLogger("ArvenComicsScraper")
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'Arven Comics'])
...
class Command(BaseCommand):
    help = 'Scrapes books from ArvenComics and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from ArvenComics.
        ...
        start_time = datetime.datetime.now()
        scraper = ArvenComicsScraper()
        try:
            scraper.scrape_arven_comics()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeArvenComics in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeArvenComics'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for ArvenComics books.")
        except Exception as e:
