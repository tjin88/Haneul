...
log_directory = "../out/SuryaScans"
log_base_filename = "scrapeSuryaScans"
...
logger = logging.getLogger("SuryaScansScraper")

class SuryaScansScraper:
    def scrape_surya_scans(self):
        # Define URLs for scraping
        url = 'https://suryatoon.com/manga/list-mode/'
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'Surya Scans'])
...
class Command(BaseCommand):
    help = 'Scrapes books from SuryaScans and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from SuryaScans.

        Executes the scraping process, calculates the duration of the operation, and logs the result.
        """
        # logger.info("Starting to scrape SuryaScans")
        ...
        start_time = datetime.datetime.now()
        scraper = SuryaScansScraper()
        try:
            scraper.scrape_surya_scans()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeSuryaScans in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeSuryaScans'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for SuryaScans books.")
        except Exception as e:
