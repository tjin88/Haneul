...
log_directory = "../out/HelScans"
log_base_filename = "scrapeHelScans"
...
logger = logging.getLogger("HelScansScraper")
    ...
    def scrape_book_and_update_db(self, title_url_tuple, book_number, total_books):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                cursor.execute("SELECT newest_chapter FROM all_books WHERE title = %s AND novel_source = %s", [normalized_title.strip(), 'Hel Scans'])
...
class Command(BaseCommand):
    help = 'Scrapes books from HelScans and updates the database.'

    def handle(self, *args, **kwargs):
        """
        Handles the command execution for scraping books from HelScans.
        ...
        start_time = datetime.datetime.now()
        scraper = HelScansScraper()
        try:
            scraper.scrape_hel_scans()

            duration = datetime.datetime.now() - start_time
            formatted_duration = self.format_duration(duration)

            logger.info(f"Successfully executed scrapeHelScans in {formatted_duration} ")
            self.stdout.write(self.style.SUCCESS('Successfully executed scrapeHelScans'))
        except ConnectionError:
            logger.error(f"Looks like the computer was not connected to the internet. \
                         Abandoned this attempt to update server for HelScans books.")
        except Exception as e:
